...
st.session_state.producccion_leche["Año"] = st.session_state.producccion_leche[
    "Fecha"
].dt.year.astype(int)
...
produccion_leche_año = st.session_state.producccion_leche[
    st.session_state.producccion_leche["Año"] == año_seleccionado
]
...
cantidad = st.number_input(
    "Cantidad (Litros)", min_value=0, max_value=1000, value=0, step=1
)

# Agregar los datos de producción de leche
if st.button("Agregar datos de producción de leche"):
    st.session_state.producccion_leche.loc[
        st.session_state.producccion_leche["Fecha"] == fecha, "Cantidad (Litros)"
    ] = cantidad
    ...
    # Leer el historial de leche más reciente de la nube para evitar sobrescribir datos
    try:
        historial_leche_nube = conn.read(worksheet="Registro_cambios_leche", ttl=0)
    except:
        historial_leche_nube = pd.DataFrame(columns=["Cambio"])
    ...
    # Nota: Concatenamos con historial_leche_nube para asegurar la integridad del log
    st.session_state.registro_cambios_leche = pd.concat(
        [df, historial_leche_nube], ignore_index=True
    ).dropna(how="all")

    # Mostrar el registro modificado de producción de leche
    st.dataframe(
        st.session_state.producccion_leche[
            st.session_state.producccion_leche["Fecha"] == fecha
        ],
        width=1500,
    )
    ...
    st.success(
        "Se agregaron los datos de producción de leche de "
