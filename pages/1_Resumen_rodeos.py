...
numero_vacas_por_rodeo = st.session_state.lista_completa_vacas.groupby('Rodeo')['NumeroRP'].count().reset_index()
...
st.data_editor(
    st.session_state.lista_completa_vacas[st.session_state.lista_completa_vacas['Rodeo'] == rodeo_seleccionado].reset_index(drop=True),
    width = 1500,
    column_config={
        "widgets": st.column_config.TextColumn(
...
# Obtener el numero de vacas por Rodeo
edades_por_rodeo = st.session_state.lista_completa_vacas[st.session_state.lista_completa_vacas['Rodeo'] == rodeo_seleccionado].groupby('Años')['NumeroRP'].count().reset_index()

# Renombrar las columnas
edades_por_rodeo.columns = ['Años', 'Numero_vacas']

barplot_edades = px.bar(edades_por_rodeo, x="Años", y ="Numero_vacas", opacity=0.8, 
                           color_discrete_sequence=px.colors.qualitative.Plotly)

barplot_edades.update_layout(
    xaxis=dict(title='Años', tickfont=dict(size=18), titlefont=dict(size=20)),
    yaxis=dict(title='Numero de vacas', tickfont=dict(size=18), titlefont=dict(size=20))
    ).update_xaxes(
        showgrid=False
    ).update_yaxes(
        showgrid=False)
...
# Obtener el numero de vacas por Rodeo
razas_por_rodeo = st.session_state.lista_completa_vacas[st.session_state.lista_completa_vacas['Rodeo'] == rodeo_seleccionado].groupby('Raza')['NumeroRP'].count().reset_index()
razas_por_rodeo.columns = ['Raza', 'Numero_vacas']

pie_plot_razas = px.pie(values=razas_por_rodeo['Numero_vacas'],
