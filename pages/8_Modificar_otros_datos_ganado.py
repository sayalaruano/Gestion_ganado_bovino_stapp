...
numero_rp = st.text_input(
    "Ingresa el NumeroRP del animal", value="", max_chars=None, key=None, type="default"
)
...
if not animal_NumeroRP.empty:
    with st.form("form_modificar_vaca"):
        ...
        nombremadre = st.text_input(
            "Nombre de la madre", value=animal_NumeroRP["Nombre_Madre"].values[0]
        )
        numeromadre = st.text_input(
            "Número de la madre", value=animal_NumeroRP["Numero_Madre"].values[0]
        )
        fecha_ultima_insem_dt = pd.to_datetime(
            animal_NumeroRP["Fecha_ultima_inseminacion"].values[0]
        )
        fecha_ultima_insem = st.text_input(
            "Fecha Última Inseminación (Año-Mes-Dia)", value=fecha_ultima_insem_dt
        )
        estado_preñez = st.selectbox(
            "Estado de preñez", ["Vacia", "Preñada", "Aborto"], index=0
        )
        observaciones_insem = st.text_area(
            "Observaciones inseminación",
            value=animal_NumeroRP["Observaciones_inseminacion"].values[0],
        )
        observaciones_nacim = st.text_area(
            "Observaciones nacimiento",
            value=animal_NumeroRP["Observaciones_nacimiento"].values[0],
        )
        fecha_ultima_med_dt = pd.to_datetime(
            animal_NumeroRP["Fecha_ultima_medicina"].values[0]
        )
        fecha_ultima_med = st.text_input(
            "Fecha Última Medicina (Año-Mes-Dia)", value=fecha_ultima_med_dt
        )
        observaciones_medic = st.text_area(
            "Observaciones medicina",
            value=animal_NumeroRP["Observaciones_medicina"].values[0],
        )
        fecha_muerte = st.text_input(
            "Fecha de Muerte (Año-Mes-Dia)",
            value=animal_NumeroRP["Fecha_muerte_venta"].values[0],
        )
        ...
        edad = st.text_input(
            "Edad (Años, Meses)", value=animal_NumeroRP["Edad"].values[0]
        )
        ...
        if submit_button:
            ...
            try:
                ...
                # Resetear el indice
                st.session_state.lista_completa_vacas.reset_index(
                ...
                try:
                    historial_nube = conn.read(
                        worksheet="Registro_cambios_ganado", ttl=0
                    )
                except:
                ...
                st.error(f"Ocurrió un error durante la modificación del resgistro: {e}")
else:
